...
class BookList(MethodView):
    @book_blp.response(200, BookSchema(many=True)) # many=True => 여러개 data라면 여러개로 다 보여줄게.
    def get(self):
        return books
...
class Book(MethodView):
    ...
    def get(self, book_id):
        book = next((book for book in books if book['id'] == book_id), None) # next 함수: 특정 조건 만족하는 data 찾으면 return, 못 찾으면 none.
        if book is None:
